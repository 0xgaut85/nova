...
const serverData = [
  { name: 'Nova402', value: 400, color: '#EAB308' },
  ...
  useEffect(() => {
    const verifyWallet = async () => {
      if (isConnected && address && signMessageAsync && !isVerified) {
        setIsVerifying(true);
        try {
          const message = `Sign this message to verify your wallet ownership for Nova402 explorer.\n\nWallet: ${address}\nTimestamp: ${new Date().toISOString()}`;
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-4">
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <h1 className="text-xl font-semibold italic text-gray-900">
              Nova402<span className="text-yellow-600">scan</span>
                </h1>
              </Link>
